...
MODEL_SET_ID = os.getenv("MODEL_SET_ID")
if MODEL_SET_ID == "qwen-image-edit-2511":
    MODELS = {
        "qwen_image_vae": {
...
elif MODEL_SET_ID == "wan-2.2":
    MODELS = {
    }
else:
    raise ValueError(f"Unknown MODEL_SET_ID: {MODEL_SET_ID}")
    import sys
    sys.exit(1)
...
def download_model(model_name, model_info):
    try:
        if os.path.exists(model_info["local_dest"]):
            print(f"Model '{model_name}' already exists at '{model_info['local_dest']}'. Skipping download.")
            return
...
with ThreadPoolExecutor(max_workers=len(MODELS)) as executor:
    for model_name, model_info in MODELS.items():
        executor.submit(download_model, model_name, model_info)
